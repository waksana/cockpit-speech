...
import { MAX_SAMPLES, MAX_SECONDS, SpeechError } from '../shared/limits.ts';

async function startRecording(signal: AbortSignal, fail: (error: SpeechError) => void, env: AudioEnvironment, limit = () => {}) {
  return prepareRecording(signal, fail, limit, env).start();
}

function fixture() {
  let stops = 0, closes = 0, portClosed = 0, disconnects = 0, captures = 0, resumed = 0;
  ...
  const context = {
    ...
    resume: async () => { resumed++; }, close: async () => { closes++; }, state: 'running', onstatechange: null,
  ...
  const env: AudioEnvironment = {
    ...
    getUserMedia: () => { captures++; return permission; }, createNode: () => node as unknown as AudioWorkletNode,
  };
  return { env, node, port, context, track, stream, grant, values: () => ({ stops, closes, portClosed, disconnects, captures, resumed }) };
}
test('gesture preparation unlocks audio without requesting microphone permission before configuration succeeds', async () => {
  const f = fixture();
  const preparation = prepareRecording(new AbortController().signal, () => assert.fail('unexpected failure'), () => {}, f.env);
  assert.equal(f.values().resumed, 1);
  assert.equal(f.values().captures, 0);
  preparation.cancel();
  assert.equal(f.values().closes, 1);
  await assert.rejects(preparation.start(), /cancelled/);
  assert.equal(f.values().captures, 0);
});
...
test('unsupported rate, insecure context and processor errors are explicit failures', async () => {
...
test('synchronous setup failures still handle rejected resume promises and release late permission grants', async () => {
  ...
  await assert.rejects(startRecording(new AbortController().signal, () => {}, unavailable.env), /无法访问麦克风/);
  ...
  await assert.rejects(startRecording(new AbortController().signal, () => {}, worklet.env), /无法访问麦克风/);
  worklet.grant(worklet.stream);
  await new Promise<void>(resolve => setImmediate(resolve));
  assert.equal(worklet.values().closes, 1);
  assert.equal(worklet.values().stops, 1);
});
test('wall-clock limit finalizes existing audio and racing stops reuse the same result', async t => {
  t.mock.timers.enable({ apis: ['setTimeout'] });
  const f = fixture();
  let stopping: Promise<Uint8Array<ArrayBuffer>> | undefined;
  let limits = 0;
  const pending = startRecording(new AbortController().signal, () => assert.fail('unexpected failure'), f.env,
    () => { limits++; stopping = recorder.stop(); });
  f.grant(f.stream);
  const recorder = await pending;
  f.port.onmessage!({ data: { type: 'chunk', samples: new Float32Array([0.1, 0.2]) } });
  f.port.postMessage = () => f.port.onmessage!({ data: { type: 'done' } });
  t.mock.timers.tick(MAX_SECONDS * 1000);
  assert.equal(limits, 1);
  assert.equal(recorder.stop(), stopping);
  const audio = await stopping!;
  validateWav(audio);
  assert.equal(audio.byteLength, 48);
  assert.equal(f.values().closes, 1);
});

test('sample-count limit preserves the final PCM and waits for its done event', async () => {
  const f = fixture();
  let stopping: Promise<Uint8Array<ArrayBuffer>> | undefined;
  const pending = startRecording(new AbortController().signal, () => assert.fail('unexpected failure'), f.env,
  ...
  f.grant(f.stream);
  const recorder = await pending;
  f.port.onmessage!({ data: { type: 'chunk', samples: new Float32Array(MAX_SAMPLES).fill(0.25) } });
  f.port.onmessage!({ data: { type: 'limit' } });
  assert.equal(recorder.stop(), stopping);
  f.port.onmessage!({ data: { type: 'done' } });
  const audio = await stopping!;
  validateWav(audio);
  assert.equal(audio.length, 44 + MAX_SAMPLES * 2);
  assert.equal(f.values().closes, 1);
});

test('actual AudioWorklet downmixes and flushes the complete two-minute recording before notifying the limit', async () => {
  ...
  assert.equal(events.at(-2)!.type, 'limit');
  assert.equal(events.at(-1)!.type, 'done');
  assert.equal(events.slice(before).reduce((count, event) => count + (event.samples?.length ?? 0), 0), MAX_SAMPLES);
  const after = events.length;
  limited.port.onmessage({ data: 'stop' });
  assert.equal(events.length, after, 'a racing manual stop does not emit a second finalization');
});
