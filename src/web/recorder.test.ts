...
function serviceFixture(f: ReturnType<typeof fixture>, editable = false) {
  let leases = 0, text = '', revision = 0;
  const draft: ModuleDraft = {
    id: 'exact-input', sessionId: 's', purpose: { kind: 'prompt' },
    subscribe: () => () => {},
    getSnapshot: () => ({ text, revision, pending: false, unconfirmed: false, hasContent: !!text, blocks: [] }),
    editText: value => { assert.ok(editable, 'Failure must not edit'); text = value; revision++; },
